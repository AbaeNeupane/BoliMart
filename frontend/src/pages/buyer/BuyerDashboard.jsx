            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              {won.map((bid) => (
                <div key={bid.id} className="p-4 border-b border-gray-100 last:border-b-0 flex items-center justify-between">
                  ...
                  <Link to={`/buyer/checkout/${bid.listing_id}`}>
              ...
              <tbody className="divide-y divide-gray-100">
                {bids.map((bid) => (
                  <tr key={bid.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4 text-gray-900 font-semibold">{formatCurrency(bid.amount)}</td>
                    <td className="px-5 py-4"><Badge status={bid.status} /></td>
